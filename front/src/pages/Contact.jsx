...
const appStyles = `
  ...
  *, *::before, *::after {
  ...
  .col-lg-7, .col-lg-5, .col-md-6, .col-lg-4, .col-lg-2, .col-lg-3 { 
    position: relative; 
    width: 100%; 
    padding-right: 15px; 
    padding-left: 15px; 
...
@media (min-width: 992px) {
 ...
 .footer { 
  ...
  .form-control:focus {
    ...
    box-shadow: 0 0 0 3px rgba(30, 108, 194, 0.15);
  }
  .btn-submit {
    width: 100%;
    padding: 15px;
 ...
 */
const ContactPage = ({ callGeminiAPI }) => {
  const [submitted, setSubmitted] = useState(false);
  const [formData, setFormData] = useState({ name: '', email: '', subject: '', message: '' });
  const [isSuggesting, setIsSuggesting] = useState(false);

  const handleSuggestSubject = async () => {
      if (!formData.message.trim() || isSuggesting) return;
      setIsSuggesting(true);
      const prompt = `حلل الرسالة التالية باللغة العربية. قم بتوفير عنوان موضوع موجز وملائم لها باللغة العربية، لا يزيد عن 5 كلمات. أجب فقط بسطر الموضوع نفسه ولا شيء آخر. الرسالة: '${formData.message}'`;
      try {
          const suggestion = await callGeminiAPI(prompt);
          setFormData(prev => ({...prev, subject: suggestion.trim()}));
      } catch (error) {
            ...
            <div className="row align-items-center">
                <div className="col-lg-7">
                    <div className="contact-hero-content">
                        ...
                        <p>
                            نؤمن بأهمية التواصل المباشر. فريقنا جاهز دائمًا للاستماع إلى استفساراتك واقتراحاتك لمساعدتنا على التطور وخدمتك بشكل أفضل.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="contact-visual">
                        <div className="info-card-3d">
                            <div className="info-card-item">
                                <div className="info-card-icon"><i className="fas fa-envelope"></i></div>
                                <div className="info-card-content">
                                    <h4>البريد الإلكتروني</h4>
                                    <p>support@sadeq.gov.eg</p>
                                </div>
                            </div>
                            <div className="info-card-item">
                                <div className="info-card-icon"><i className="fas fa-phone"></i></div>
                                <div className="info-card-content">
                                    <h4>الخط الساخن</h4>
                                    <p>155</p>
                                </div>
        ...
        <section className="contact-form-section">
          <div className="container">
            <div className="contact-form-wrapper">
              <h2 className="mb-4 text-center">أرسل لنا رسالة مباشرة</h2>
              {submitted ? (
                  ...
                  <div className="row">
                      <div className="col-md-6">
                          <div className="form-group">
                              <input type="text" name="name" className="form-control" placeholder="الاسم الكامل" value={formData.name} onChange={handleChange} required />
                          </div>
                      </div>
                      <div className="col-md-6">
                          <div className="form-group">
                              <input type="email" name="email" className="form-control" placeholder="البريد الإلكتروني" value={formData.email} onChange={handleChange} required />
                          </div>
                      </div>
                  </div>
                  <div className="form-group">
                    <input type="text" name="subject" className="form-control" placeholder="الموضوع" value={formData.subject} onChange={handleChange} required />
                    <button type="button" className="btn-ai-suggest" onClick={handleSuggestSubject} disabled={isSuggesting}>
                        {isSuggesting ? '...' : '✨ اقترح موضوعًا'}
                    </button>
                  </div>
                  <div className="form-group">
                    <textarea name="message" className="form-control" rows="6" placeholder="رسالتك" value={formData.message} onChange={handleChange} required></textarea>
                  </div>
...
const PrivacyPage = () => <div className="page-container"><div className="container" style={{height: '100vh'}}><h1>سياسة الخصوصية</h1></div></div>;
